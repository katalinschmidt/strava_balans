...
window.onload = () => {
    ...
    function customizeTiles(userCustomizedVal) {
        // Remove existing tile layer then repopulate:
        tilesLayer.eachLayer((tiles) => {
            tiles.remove();
        });  
...
//     // Create event listeners / handlers for filters:
    ...
    //     $('#custom-contrast').html($('#contrast-slider').val());
    //     let userCustomizedVal = `${$('#custom-contrast').text()}%`;
    //     // console.log(userCustomizedVal); // OUTPUT = good, but no change to map (w/ & w/o customFilter in this func)
    ...
    //         'saturate: 175%'
    //     ]
    //     // console.log(customFilter); // OUTPUT = good, but no change to map
    // });
// }